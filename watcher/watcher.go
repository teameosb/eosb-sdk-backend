...
func (w *Watcher) Run() {
	...
	for {
		...
		case <-w.Ctx.Done():
			return
		default:
			currentBlockNumber, err := w.Eosb.GetBlockNumber()
			...
			err = w.KVClient.Set(common.EOSB_WATCHER_BLOCK_NUMBER_CACHE_KEY, strconv.FormatUint(w.lastSyncedBlockNumber, 10), 0)
...
func (w *Watcher) initBlockNumber() {
	...
	val, err := w.KVClient.Get(common.EOSB_WATCHER_BLOCK_NUMBER_CACHE_KEY)

	if err == common.KVStoreEmpty {
		blockNumber, _ = w.Eosb.GetBlockNumber()
...
func (w *Watcher) syncNextBlock() (err error) {
	...
	block, err := w.Eosb.GetBlockByNumber(w.lastSyncedBlockNumber + 1)
